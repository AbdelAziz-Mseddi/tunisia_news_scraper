...
own. The current `MosaiquePlaywrightScraper` starts from the public
news sitemap, then only falls back to the listing page if needed. Three
ways to get URLs:

1. **`MosaiquePlaywrightScraper.discover_article_urls()`** — uses the
  public news sitemap first and only falls back to the listing page if
  needed. Slowest but most self-contained; see `uv run tunisia-scrape
  --with-mosaique-playwright`.
2. **Check other sitemap feeds** once you have real network access —
  many Next.js news sites expose separate sitemap sections for news,
  rubriques, podcasts, and videos. This project currently uses the
  news sitemap because it contains the article URLs we want.
3. **Google/Bing site-search** as a manual/occasional backfill method.
